...
def rename_df_columns(df_sumstats):
    ...
    bp_column = find_df_column(df_sumstats, ['BP', 'POS', 'POSITION', 'COORDINATE', 'BASEPAIR', 'BASE_PAIR_LOCATION'])
    snp_column = find_df_column(df_sumstats, ['SNP', 'RSID', 'RS', 'NAME', 'MarkerName', 'VARIANT_ID'])
    a1freq_col = find_df_column(df_sumstats, ['A1FREQ', 'all_meta_AF', 'freq', 'MAF', 'FRQ', 'EFFECT_ALLELE_FREQUENCY'], allow_missing=True)
    ...
    se_col = find_df_column(df_sumstats, ['SE', 'inv_var_meta_sebeta', 'STANDARD_ERROR'], allow_missing=True)
    pvalue_col = find_df_column(df_sumstats, ['P_BOLT_LMM', 'P', 'inv_var_meta_p', 'PVALUE', 'P-VALUE', 'P_value', 'PVAL', 'P_VALUE'], allow_missing=True)
    ...
    ncase_col = find_df_column(df_sumstats, ['N_cases', 'N_case', 'Ncase', 'Nca','Total_NCase'], allow_missing=True)    
    ncontrol_col = find_df_column(df_sumstats, ['N_controls', 'N_ctrl', 'Ncontrol','Nco','Total_NControl'], allow_missing=True)    
    try:
        allele1_col = find_df_column(df_sumstats, ['ALLELE1', 'A1', 'a1', 'a_1', 'ALT', 'EFFECT_ALLELE'])
        allele0_col = find_df_column(df_sumstats, ['ALLELE0', 'A0', 'a0', 'a_0', 'REF', 'OTHER_ALLELE'])
    except ValueError:
